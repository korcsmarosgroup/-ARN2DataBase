            ...
            if line is None:
                ...
                with conn:
                    ...
                    if layer == 0:
                        ...
                        # If node is ATG protein we check all molecule types (Layers for interactions) - These will be L' interactions
                        # USE THIS AFTER ONLY L0 IS PARSED
                        if line['interactor_a_node_name'] in nodes_added_in_previous_layers or \
                                line['interactor_b_node_name'] in nodes_added_in_previous_layers:
                            layer_remaining_counter[layer] += 1
                            insert_new_edge(c, line)


def build_whole(log, path):
    # Creating output table for EDGES
    path = '../DATA/workflow/merger.db'
