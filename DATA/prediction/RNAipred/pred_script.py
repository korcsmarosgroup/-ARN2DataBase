                        ...
                        for gene in score_dict:
                            ...
                            try:
                                ...
                                if score_value >= 1:
                                    ...
                                    score_value = 0
                                df.set_value(gene, pheno_dict[gene], score_value)
        ...
        for layer in layer_list:
            ...
            while True:
                ...
                if edge is None:
                    ...
                    for index in df:
                        try:
                            A_node_score = df.get_value(A_node, index)
                            B_node_score = df.get_value(B_node, index)
                        except KeyError:
                            error_count += 1
                            continue
                        ...
                        if pos_corr + neg_corr >= 1:
                            ...
                            c2.execute("UPDATE layer%d SET confidence_scores = layer%d.confidence_scores || %s"
                                       ...
                                       % (layer, layer, '|sign_pred:' + str(sign_score) + '|', edge[0], edge[1]))
                            notcut_count += 1
                        else:
                            cutoff_count += 1
